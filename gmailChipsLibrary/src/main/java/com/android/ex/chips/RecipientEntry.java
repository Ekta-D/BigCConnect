...
public class RecipientEntry {
	...
	/**
	 * A GENERATED_CONTACT is one that was created based entirely on information
	 * passed in to the RecipientEntry from an external source that is not a
	 * real contact.
	 */
	/* package */static final int GENERATED_CONTACT = -2;
	/**
	 * Used when {@link #mDestinationType} is invalid and thus shouldn't be used
	 * for display.
	 */
	...
	/**
	 * True when this entry is the first entry in a group, which should have a
	 * photo and display name, while the second or later entries won't.
	 */
	private final boolean mIsFirstLevel;
	private final String mDisplayName;
	/**
	 * Destination for this contact entry. Would be an email address or a phone
	 * number.
	 */
	...
	/**
	 * Label of the destination which will be used when type was
	 * {@link Email#TYPE_CUSTOM}. Can be null when {@link #mDestinationType} is
	 * {@link #INVALID_DESTINATION_TYPE}.
	 */
	...
	/**
	 * This can be updated after this object being constructed, when the photo
	 * is fetched from remote directories.
	 */
	...
	private RecipientEntry(final int entryType, final String displayName,
		...
		mDestination = destination == null ? "" : destination;
	...
	/**
	 * Determine if this was a RecipientEntry created from recipient info or an
	 * entry from contacts.
	 */
	public static boolean isCreatedRecipient(final long id) {
		return id == RecipientEntry.INVALID_CONTACT
	...
	public static RecipientEntry constructFakeEntry(final String address,
		...
		final String tokenizedAddress = tokens.length > 0 ? tokens[0]
				.getAddress() : address;
		return new RecipientEntry(ENTRY_TYPE_PERSON, tokenizedAddress,
				tokenizedAddress, INVALID_DESTINATION_TYPE, null,
				INVALID_CONTACT, INVALID_CONTACT, null, true, isValid,
				false /* isGalContact */, null);
	}

	/**
	 * Construct a RecipientEntry from just a phone number.
	 */
	public static RecipientEntry constructFakePhoneEntry(
			final String phoneNumber, final boolean isValid) {
		return new RecipientEntry(ENTRY_TYPE_PERSON, phoneNumber, phoneNumber,
				INVALID_DESTINATION_TYPE, null, INVALID_CONTACT,
				INVALID_CONTACT, null, true, isValid, false /* isGalContact */,
				null);
	}

	/**
	 * @return the display name for the entry. If the display name source is
	 *         larger than {@link DisplayNameSources#PHONE} we use the contact's
	 *         display name, but if not, i.e. the display name came from an
	 *         email address or a phone number, we don't use it to avoid
	 *         confusion and just use the destination instead.
	 */
	private static String pickDisplayName(final int displayNameSource,
			final String displayName, final String destination) {
		return displayNameSource > DisplayNameSources.PHONE ? displayName
				: destination;
	}
	...
	public static RecipientEntry constructGeneratedEntry(final String display,
		...
		return new RecipientEntry(ENTRY_TYPE_PERSON, display, address,
				INVALID_DESTINATION_TYPE, null, GENERATED_CONTACT,
				GENERATED_CONTACT, null, true, isValid, false /* isGalContact */,
				null);
	}

	public static RecipientEntry constructTopLevelEntry(
			final String displayName, final int displayNameSource,
			final String destination, final int destinationType,
			final String destinationLabel, final long contactId,
			final long dataId, final Uri photoThumbnailUri,
		...
		return new RecipientEntry(ENTRY_TYPE_PERSON, pickDisplayName(
				...
				photoThumbnailUri, true, isValid, isGalContact, null);
	}
	...
	public static RecipientEntry constructTopLevelEntry(
		...
		return new RecipientEntry(ENTRY_TYPE_PERSON, pickDisplayName(
				displayNameSource, displayName, destination), destination,
				destinationType, destinationLabel, contactId, dataId,
				thumbnailUriAsString != null ? Uri.parse(thumbnailUriAsString)
						: null, true, isValid, isGalContact, user);
	}
	...
	public static RecipientEntry constructSecondLevelEntry(
		...
		return new RecipientEntry(ENTRY_TYPE_PERSON, pickDisplayName(
				...
				destinationType, destinationLabel, contactId, dataId,
				thumbnailUriAsString != null ? Uri.parse(thumbnailUriAsString)
						: null, false, isValid, isGalContact, null);
	}
	...
	public boolean equals(final Object o) {
		...
		return o != null && mDestination.equals(other.mDestination)
	...
	public String getDestination() {
		return mDestination == null ? "" : mDestination;
	}
