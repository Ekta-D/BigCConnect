...
public class FetchNYTFeedsRequest extends SpringAndroidSpiceRequest<Feeds> {

	private Context context;

	public FetchNYTFeedsRequest(Context context) {
		super(Feeds.class);
	...
	public Feeds loadDataFromNetwork() throws Exception {
		...
		String url = "http://rss.nytimes.com/services/xml/rss/nyt/Research.xml";
						...
						while (eventType != XmlPullParser.END_TAG
							...
							if (eventType == XmlPullParser.START_TAG
								...
								if (eventType == XmlPullParser.TEXT)
									...
									&& "content".equals(xpp.getName())) {
								feed.image = xpp.getAttributeValue(3);
		...
		Feeds oldData = Preferences.getInstance(context).getFeeds("nyt");
		...
		if (feeds.size() > 0) {
			...
			Preferences.getInstance(context).saveFeeds(oldData, "nyt");
		}
		return oldData;
	}
}