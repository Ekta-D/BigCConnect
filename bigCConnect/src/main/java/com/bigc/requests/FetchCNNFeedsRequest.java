...
public class FetchCNNFeedsRequest extends SpringAndroidSpiceRequest<Feeds> {

	private Context context;

	public FetchCNNFeedsRequest(Context context) {
		super(Feeds.class);
	...
	public Feeds loadDataFromNetwork() throws Exception {
		...
		String url = "http://rss.cnn.com/rss/cnn_health.rss";
					...
					if (eventType == XmlPullParser.START_TAG
						...
						while (eventType != XmlPullParser.END_TAG
							...
							if (eventType == XmlPullParser.START_TAG
								...
								eventType = xpp.next();
								if (eventType == XmlPullParser.TEXT)
									feed.description = xpp.getText().substring(
											0, xpp.getText().indexOf("<"));
		...
		Feeds oldData = Preferences.getInstance(context).getFeeds("cnn");
		Log.e("CNN Feeds Size", feeds.size() + " -- "
		...
		if (feeds.size() > 0) {
			...
			Preferences.getInstance(context).saveFeeds(oldData, "cnn");
		}
		return oldData;
	}
}